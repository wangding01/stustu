...
import com.shuttle.alipay.PayUtil;
...
public class WXPayPrecreateController {
	...
	/**
	 * 扫码支付 - 统一下单 相当于支付不的电脑网站支付
	 *
	 * <a href=
	 * "https://pay.weixin.qq.com/wiki/doc/api/native.php?chapter=9_1">扫码支付API</a>
	 */
	@PostMapping("/order")
	public void precreate(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		reqData.put("notify_url", "http://mnx597.natappfree.cc/wxpay/precreate/notify");
	...
	public void precreateNotify(HttpServletRequest request, HttpServletResponse response) throws Exception {
		 ...
		 * nonce_str=aaaf3fe4d3aa44d8b245bc6c97bda7a8, bank_type=CFT, openid=xxx,
		 * sign=821A5F42F5E180ED9EF3743499FBCF13, fee_type=CNY, mch_id=xxx, cash_fee=1,
		 * out_trade_no=186873223426017, appid=xxx, total_fee=1, trade_type=NATIVE,
		 * result_code=SUCCESS, time_end=20180618131247, is_subscribe=N,
		 * return_code=SUCCESS }
		 */
		...
		if (signatureValid) {
			/**
			 * 注意：同样的通知可能会多次发送给商户系统。商户系统必须能够正确处理重复的通知。 推荐的做法是，当收到通知进行处理时，首先检查对应业务数据的状态，
